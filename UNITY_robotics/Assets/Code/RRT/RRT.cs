...
namespace RRT
{
    public class RRT : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            startingTime = DateTime.Now.ToString("hh: mm:ss.fff");
        ...
        private void FixedUpdate()
        {
            ...
            if (!iterateAutomatically)
            {
                if (makeOneIteration)
                {
                    Debug.Log("Itera di uno");
                    makeOneIteration = false;
                    var newNode = _rrtSearchStrategy.AddOneNodeToTree();
                    if (newNode != null) //Debug.Log("Added new Node #" + _counter.ToString() + " at pos" + newNode.Position.ToString());
                        _counter++;
                }
            }
            else
